...
module.exports = {
	name: 'fetch-alts',
  aliases: ['alt'],
  description: 'Fetch an alt account with providing a number.',
  usage: 'fetch-alts <number>',
    ...
    if (!alts.length) return message.channel.send(":x: | **No alts found**");
		...
		let e = new Discord.MessageEmbed()
			...
			.setFooter(`Page ${page + 1} of ${pages.length} (${alts.length} entries)`)
			.setColor('7289da');
			...
			if (r.emoji.name == symbols[0]) {
				if (!pages[page + 1])
					...
					let newEmbed = new Discord.MessageEmbed()
						.setDescription(pages[page].join('\n'))
						.setFooter(
							`Page ${page + 1} of ${pages.length} (${alts.length} entries)`
						)
						.setColor('7289da');
					msg.edit(newEmbed);
				}
